...
public interface CiqdNodeRepository extends ArangoRepository<CiqdNode, String> {
    ...
    @Query("FOR v,e,p IN 1..100 OUTBOUND @id @@edgeCol OPTIONS{uniqueEdges: 'path', uniqueVertices: 'path'} FILTER v._id==@final  RETURN TO_STRING(p.vertices[*]._key)")
    //Collection<String> getNoLoopsPath(@Param("id") String id, @Param("final") String finalId, @Param("@edgeCol") Class<?> edgeCollection);
    Collection<String> getNoLoopsPath(@Param("id") String id, @Param("final") String finalId, @Param("@edgeCol") Class<?> edgeCollection);
